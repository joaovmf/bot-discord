...
const getWiki = async (channel, search) => {
  ...
  try {
    ...
    channel.send(new MessageEmbed().setFooter(`Font: ${page.raw.fullurl}`).setURL(page.raw.fullurl).setTitle(info.name).setDescription(`${content[0].content.substring(0,4090)}...`).setImage(images.find(image => image.includes(imageName))))
  } catch (error) {
